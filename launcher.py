...
def launch_flask(internal_dir: Path) -> subprocess.Popen:
    """Lanza app.py como subproceso en el directorio _internal/."""
    app_path = internal_dir / "app.py"

    if getattr(sys, "frozen", False):
        # En modo compilado: usar el intérprete embebido
        python_exe = Path(sys.executable).parent / "_internal" / "python.exe"
    ...
    log.info(f"Flask lanzado (PID {proc.pid})")
    return proc
